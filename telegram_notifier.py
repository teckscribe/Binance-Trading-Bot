...
_STRATEGY_META = {
    ...
    "TSMOM_4H":   {"emoji": "\U0001f535", "label": "TSMOM 4H Trend"},
    "REBALANCING_PREMIUM": {"emoji": "\U0001f7e3", "label": "Rebalancing Premium"},
}
...
def notify_startup(regime: dict, n_symbols: int, n_scan: int = 0, permitted_strategies: list = None) -> None:
    ...
    # `is not None`, not `or`: an empty list means "nothing permitted" (every
    # strategy /disabled, or none allowed in this regime) and must print as
    # none. `or` treated [] as falsy and fell back to listing every strategy
    # as active — the exact opposite of the truth.
